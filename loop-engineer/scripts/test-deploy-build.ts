...
async function main() {
  ...
  // 5. ultrareview R4:assertNodejsCompat 必须读回 result 断言 flag 真生效(success≠生效)。
  // 命中:production.compatibility_flags 含 nodejs_compat → 不抛。
  assertNodejsCompat({ deployment_configs: { production: { compatibility_flags: ["nodejs_compat"] } } }, "ok");
